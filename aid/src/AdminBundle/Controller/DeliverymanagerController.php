...
class DeliverymanagerController extends BaseController
{
    ...
    public function indexAction()
    {
    	if(!empty($this->get('session')->get('domain_id')) && $this->get('session')->get('role_id')== '1'){
			$deliverymanager_list = $this->getDoctrine()
					   ...
					   ->findBy(array('user_role_id'=>5,'is_deleted'=>0));	
		}else{
			$deliverymanager_list = $this->getDoctrine()
					   ->getManager()
					   ->getRepository('AdminBundle:Usermaster')
					   ->findBy(array('user_role_id'=>5,'is_deleted'=>0,"domain_id"=>$this->get('session')->get('domain_id')));
		}
		
		return array("deliverymanager_list"=>$deliverymanager_list);
	}
	/**
     * @Route("/createdeliverymanager/{user_master_id}",defaults = {"user_master_id" = ""})
     * @Template()
     */
    public function createdeliverymanagerAction($user_master_id)
    {
		...
		if(!empty($this->get('session')->get('domain_id')) && $this->get('session')->get('role_id')!= '1')
		{
			...
			$delivery_admin = $this->getDoctrine()
					   ->getManager()
					   ->getRepository('AdminBundle:Usermaster')
					   ->findBy(array('is_deleted'=>0,"domain_id"=>$this->get('session')->get('domain_id'),'user_role_id'=>4));
		}
		else{
			$domain_master = $this->getDoctrine()
						   ->getManager()
			...
			$delivery_admin = $this->getDoctrine()
					   ->getManager()
					   ->getRepository('AdminBundle:Usermaster')
					   ->findBy(array('is_deleted'=>0,'user_role_id'=>4));
		}
		...
		if(!empty($user_master_id)){
			...
			$area_list = $this->getDoctrine()
				...
				return array("language"=>$language,"city_list"=>$city_list,"area_list"=>$area_list,"country_master"=>$country_master,"state_master"=>$state_master,"user_master_id"=>$user_master_id,"user_master"=>$user_master,"address_master"=>$address_master,"main_area_id"=>$main_area_id,"main_city_id"=>$main_city_id,"main_state_id"=>$main_state_id,"main_country_id"=>$main_country_id,"domain_id"=>$domain_id,"domain_list"=>$domain_master,"main_company_id"=>$main_company_id,"company_master"=>$company_master,"main_branch_id"=>$main_branch_id,"branch_master"=>$branch_master,"delivery_admin"=>$delivery_admin);
			
		}
		
		return array("language"=>$language,"country_master"=>$country_master,"user_master_id"=>$user_master_id,"domain_list"=>$domain_master,"delivery_admin"=>$delivery_admin);
	}
	/**
     * @Route("/savedmanager/{user_master_id}",defaults = {"user_master_id" = ""})
     */
    public function savedmanagerAction($user_master_id)
    {
		//button click
			if(isset($_POST['save_dmanager']) && $_POST['save_dmanager'] == "save_dmanager")
			{
				...
				if(!empty($user_master_id)){
					if($_POST['first_name'] != "" && $_POST['last_name'] != "" && $_POST['email_address'] != "" && $_POST['username'] != "")
					{
						...
						if(!empty($usermaster)){
							...
							$usermaster->setParent_user_id($_POST['d_admin']);
						...
						foreach($language as $key=>$val){
							...
							$addressmaster->setAddress_type("house");
							...
							$addressmaster->setDomain_id($_POST['main_domain_id']);
					...
					if($_POST['pass'] == $_POST['confirm_pass'])
					{
						...
						if($extension == 'jpg' || $extension == 'png' || $extension == 'jpeg')
						{
							...
							if($media_id != "FALSE")
							{
								...
								$usermaster->setCountry_id($_POST['main_country_id']);
								$usermaster->setState_id($_POST['main_state_id']);
								$usermaster->setParent_user_id($_REQUEST['d_admin']);
			...
			return $this->redirect($this->generateUrl('admin_deliverymanager_index',array("domain"=>$this->get('session')->get('domain'))));
		}
	}
	
	/**
     * @Route("/delmanageruser/{user_master_id}",defaults = {"user_master_id" = ""})
     */
